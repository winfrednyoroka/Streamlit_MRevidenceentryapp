...
pointestimate_calc <- function(est) {
  dbp = log(81.80)#obtain the log of mean DBP
  point_est <- exp((est/100) * dbp)
  return(point_est)
}
CI_calc_lower_se <- function(lowerest) {
  dbp = log(81.80)
  lowerinterval <- exp((lowerest/100) * dbp)
  return(lowerinterval)
}

CI_calc_upper_se <- function(upperest) {
  dbp = log(81.80)
  upperinterval <- exp((upperest/100) * dbp)
  return(upperinterval)
}
#Calculate the scaling factor
frank_calc_sd <- function() {
    one_percmeanBMI = exp((1/100) * log(27.3224))
    sd_BMI_UKB = 4.77
...
Wes_2022 <- P2 %>%
  ...
  mutate(lowerinterval = round(CI_calc_lower(effectsize,
                                       pvalue),2)) %>%
  mutate(upperinterval = round(CI_calc_upper(effectsize,
                                       pvalue),2))
...
Frank_2018 <- P3 %>%
  ...
  mutate(lowerinterval = CI_calc_lower_se(lowerinterval)/scalingfactor) %>%
  mutate(upperinterval = CI_calc_upper_se(upperinterval)/scalingfactor) %>% 
...
Nicholas et al adjusted for antihypertensive use by adding 5 mm Hg to DBP. Similar scaling as to what we applied on SBP was implemented here.
Frank Windmeijer convert that between scales, Percentage change in DBP due to a 1% change in BMI. Calculate the CIs from the point estimate and standard errors point estimate in mm Hg percentage change * Mean DBP in UKBiobank (perc/100) * 81.80 CI= estimate+_margin_error margin of error at 95perc = 1.96xse. calculate the SD at 1% change in BMI. 1% of average BMI in UKBiobank is 0.273224kg/m^2, sd at 1perc = 0.273224 divide by 4.77 (sd of BMI in UKBiobank). Scale the change in DBP to 1 SD: estimates/((0.273224/4.77)), one_percmeanBMI = (1/100) * 27.3224, sd_BMI_UKB = 4.77, sd_at_1%ofBMI =  one_%meanBMI/sd_BMI_UKB, At 1 SD, what is the change in DBP in mm Hg:pointestimate/sd_at_1%ofBMI  
